...
하지만 아쉬운 점은 분명히 존재하는데요. 무엇보다 가장 큰 문제점은 사람의 실수가 서비스에 바로 반영된다는 것입니다. 기본적으로 버전관리에 대한 이야기인데요. 일반적인 개발 코드들은 일련의 품질검사(QA) 과정을 거친 후에 프로덕션 배포가 이루어지지만 번역 파일은 배포 채널이 프로덕션 단 하나만 존재하기 때문에 배포와 동시에 라이브된다는 특징을 갖고 있습니다.

이러한 단점들을 모두 커버하려면 결국은 탈 스프레드시트를 해야하는 시점이 올텐데요. 궁극적으로는 한국에서 정상적으로 사용 가능한 Lokalize와 같은 서비스가 빨리 나왔으면 하는 바램입니다. 개인적으로 이와 관련된 오픈소스 프로젝트를 준비 중이기도 한데요. 나중에 소개할 수 있는 때가 오면 다시 한 번 소개해 보도록 하겠습니다.
