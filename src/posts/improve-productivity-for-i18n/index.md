...
이번 글에서는 신규 글로벌 사업 준비에 발맞춰 개발팀이 어떤 준비를 했으며, 외부 인력은 번역가들과의 협업을 어떤 식으로 풀어 나갔는지 그 히스토리를 공유해보려고 합니다. 본문에서 소개하는 내용들은 TOAST 기술 블로그에 올라온 [국제화(i18n) 자동화 가이드](https://ui.toast.com/weekly-pick/ko_20210303)에서 많은 도움을 받았습니다.
...
[Lokalize](https://lokalise.com/)는 다국어 대응을 위한 자동화 도구이자 플랫폼입니다. 국제화 기술 개발을 위한 리서치 중 구글스프레드시트와 함께 찾은 또 다른 대안이었는데요. 대표적인 기능으로 여러사람이 한 번에 번역 작업에 참여할 수 있는 플랫폼이면서, 번역된 결과물을 미리 확인해볼 수 있는 프리뷰 기능, Figma와 같은 외부 디자인 툴에서 번역물을 불러와 시안에 적용할 수 있는 기능과 같이 다양한 부가 기능을 제공합니다.

사실상 저희가 구글 스프레드시트와 자체 인터널 서비스를 통해 구축한 시스템을 이미 프로덕트로 만들어놓은 셈이었는데요. 도입이 어려웠던 이유는 이미 저비용으로 구축된 환경에서 비용을 지불하고 해당 서비스를 이용해야할 만큼의 가치를 못느꼈다는 점과 실제로 사용해보면 서비스가 매우 느리다는 점입니다. 국제화 작업 특성상 빠르게 조회하고 빠르게 수정, 배포할 수 있어야 하는데 Lokalize는 해외 리전만 존재하는 것인지 서버 반응이 매우 느렸습니다.
