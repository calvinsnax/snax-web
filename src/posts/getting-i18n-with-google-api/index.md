...
따라서 번역물만 별도로 배포할 수 있는 시스템을 구축해야 했는데요. 저희는 이를 **국제화 원격 배포 시스템**라고 불렀습니다.

![국제화 파일 원격 배포 시스템](./locale-manager.png)

**국제화 원격 배포 시스템**은 [Next.js](https://nextjs.org/)로 개발되었는데요. Next.js를 선정한 이유에는 SSR이기 때문에 로직이 거의 노출되지 않는다는 점과 API Page 기능을 통해 백엔드 API를 개발할 수도 있기 때문입니다.
